...
def add_subtasks(args):
    """Bir göreve alt görevler ekle"""
    conn = get_connection()
    cursor = conn.cursor()
    
    # Ana görevi kontrol et
    cursor.execute("SELECT * FROM tasks WHERE id = ?", (args.parent_id,))
    parent_task = cursor.fetchone()
    
    if not parent_task:
        print(f"Hata: {args.parent_id} ID'li görev bulunamadı.")
        conn.close()
        return
    ...
    # Alt görevleri ekle (TASK-001-subtasks.md dosyasından)
    if args.file:
        try:
            import re
            ...
            cursor.execute("UPDATE tasks SET has_subtasks = 1 WHERE id = ?", (args.parent_id,))
            ...
            for subtask_id, subtask_content in subtask_blocks:
                ...
                if subtask_exists:
                    ...
                    WHERE subtask_id = ?
                    """, (title, description, status, priority, estimated_hours, subtask_id))
                else:
                    # Yeni alt görev ekle
                    cursor.execute("""
                    ...
                    """, (args.parent_id, subtask_id, title, description, status, priority, estimated_hours))
            ...
            print(f"{subtask_count} alt görev {args.parent_id} görevine eklendi.")
        ...
        # Manuel alt görev eklemesi ileride eklenebilir
        print("Not: Şu anda alt görev eklemek için --file parametresi gereklidir.")
...
def main():
    ...
    # Alt görev ekleme komutu
    subtasks_parser = subparsers.add_parser("add-subtasks", help="Bir göreve alt görevler ekle")
    subtasks_parser.add_argument("parent_id", help="Ana görev ID")
    subtasks_parser.add_argument("--file", help="Alt görevleri içeren markdown dosyası")
    ...
    if args.command == "list":
        ...
        show_sessions(args)
    elif args.command == "add-subtasks":
        add_subtasks(args)
    else:
        parser.print_help()
